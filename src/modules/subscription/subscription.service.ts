...
const TIER_ENTITLEMENTS: Record<SubscriptionTier, {
  genericTryOn: boolean;
  ownPhotoTryOn: boolean;
  freeGenerationLimit: number | null;
}> = {
  free_trial: { genericTryOn: true, ownPhotoTryOn: false, freeGenerationLimit: 2 },
  basic: { genericTryOn: true, ownPhotoTryOn: false, freeGenerationLimit: null },
  pro: { genericTryOn: true, ownPhotoTryOn: true, freeGenerationLimit: null },
  gold: { genericTryOn: true, ownPhotoTryOn: true, freeGenerationLimit: null },
};
...
export class SubscriptionService {
  ...
  async cancelSubscription(userId: string) {
    ...
    if (!current) throw new AppError(404, "No active subscription found for this account");
    if (current.cancel_at_period_end) return current;
    const subscription = await this.subscriptionRepo.updateById(current.id, { cancel_at_period_end: true });
    if (!subscription) throw new AppError(404, "No subscription found for this account");
    return subscription;
  }
  ...
  async expireLapsedSubscriptions(): Promise<number> {
    ...
    await Promise.all(lapsed.map(async (subscription) => {
      const status = subscription.cancel_at_period_end ? "cancelled" : "expired";
      await this.subscriptionRepo.updateById(subscription.id, { status });
      logger.info({ userId: subscription.user_id, status }, "Subscription period ended");
    }));
