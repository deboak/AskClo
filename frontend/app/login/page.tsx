...
export default function LoginPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const session = await api<AuthData>("/auth/login", {
        method: "POST",
        body: JSON.stringify({
      ...
      <section className="authAside">
        <div>
          <span className="kicker">
            <span>✦</span> Styled around you
          </span>
          <h1>
            Good style starts with being <em>understood.</em>
          </h1>
          <p>Come back to your saved preferences, conversations and outfit edits.</p>
        </div>
        <span className="asideNote">ASKCLO · LAGOS</span>
      </section>
      <section className="authFormWrap">
        <form className="authForm" onSubmit={submit}>
          <span className="kicker dark">Welcome back</span>
          <h2>Sign in to your wardrobe</h2>
          <p>
            New to AskClo? <Link href="/register">Create an account</Link>
          </p>
